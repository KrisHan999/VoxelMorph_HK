...
class SpatialTransformer(Layer):
    ...
    def build(self, input_shape):
        ...
        # input_shape[0] -> src.shape=[None,160,192,224,1]
        # input_shape[1] -> tgt.shape=[None,160,192,224,1]
        self.ndims = len(input_shape[0]) - 2 # -> 3
        self.inshape = input_shape
        vol_shape = input_shape[0][1:-1] # vol_shape=[160,192,224]
        trf_shape = input_shape[1][1:]  # trf_shape=[160,192,224,3]

        # print("input_shape")
        # print(input_shape[0])  #(None, 160, 192, 224, 1)
        # print(input_shape[1])  #(None, 160, 192, 224, 3)
    ...
    def call(self, inputs):
        ...
        # inputs[0] -> src,  shape=?x160x192x224x1
        # inputs[1] -> tgt,  shape=?x160x192x224x1
        vol = inputs[0]
        trf = inputs[1]

        # print('shape')
        # print(inputs[0].shape)  (?, 160, 192, 224, 1)
        # print(inputs[1].shape)  (?, 160, 192, 224, 3)

        # necessary for multi_gpu models...
        vol = K.reshape(vol, [-1, *self.inshape[0][1:]]) 
        trf = K.reshape(trf, [-1, *self.inshape[1][1:]]) 

        # print(vol.shape) (?, 160, 192, 224, 1)
        # print(trf.shape) (?, 160, 192, 224, 3)
